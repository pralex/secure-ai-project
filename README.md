...
Проект построен по принципу минимума прав. Агент видит только исходники,
изолирован от базы данных и запускается без root. Сеть ему **доступна** — она
нужна для локального инференса (Ollama), каталога моделей и опциональных облачных
провайдеров.
...
- Лишён capabilities, запускается без root. Сеть доступна (Ollama + каталог моделей).
- База данных недоступна (внутренняя сеть `db-net`).
